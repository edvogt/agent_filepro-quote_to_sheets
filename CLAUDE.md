...
1. **GoogleSheetsClient** (line 88): OAuth token loading/refresh, Sheets CRUD operations, formatting. Uses `gspread` library.
   - `find_sheet_by_quote_number()` (line 107): Searches existing sheets in target folder
   - `create_or_update_sheet()` (line 125): Main entry point for sheet creation/update
   - `_populate_worksheet()` (line 163): Writes header, line items, and financial summary
   - `_apply_formatting()` (line 217): Applies colors, bold headers, auto-resize, freeze rows

2. **QuotationProcessor** (line 282): JSON parsing, quote number extraction from filename, data cleaning with pandas, coordinates sync workflow.
   - `_fix_json_file()` (line 289): Calls `fix_filepro_json.sh` to fix malformed JSON before parsing
   - `_parse_filepro_json()` (line 322): Regex-based parser for malformed FilePro JSON (fallback)
   - `_convert_filepro_metadata()` (line 422): Transforms FilePro structure to standard format
   - `process_file()` (line 467): Main entry point - fixes JSON, handles trigger files, all formats

3. **QuotationFileHandler** (line 638): Watchdog `FileSystemEventHandler` subclass. Monitors export directory, debounces file creation events (2-second delay), prevents duplicate processing via `self.processing` set.
...
- `call_webhook()` (line 246): POST to Apps Script after successful sync
...
`fix_filepro_json.sh` automatically fixes malformed FilePro JSON before processing. Called by `_fix_json_file()` (line 289) in `QuotationProcessor`.
...
Filename pattern: `QUOTE_[NUMBER]_[TIMESTAMP].json` (e.g., `QUOTE_12345_20250124_143022.json`) - quote number extracted from second underscore segment via `_extract_quote_number()` (line 585).
...
On startup, `process_existing_files()` (line 673) processes any files already in the watch directory before starting the watchdog observer. The observer then monitors for new files in real-time.
